...
def unit_test(plural_table,test=nil)
	...
	sources=unit_sources(singular_table)
	not_uptodate_sources=sources.select {|s| !File.exist?(target) ||  File.exist?(s) && !uptodate?(target, s)}
	stop=ruby_run_and_log("test/unit/#{singular_table}_test.rb", target,test)
	if !stop then
		git_add_successful(not_uptodate_sources)
	end #if
	return stop
end #def
...
def git_add_successful(not_uptodate_sources)

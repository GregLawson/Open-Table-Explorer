class AcquisitionsController < ApplicationController
  ...
  def index
    @acquisitions = Acquisition.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @acquisition.save
        flash[:notice] = 'Acquisition was successfully created.'
        format.html { redirect_to(@acquisition) }
  ...
  def update
    ...
    respond_to do |format|
      if @acquisition.update_attributes(params[:acquisition])
        flash[:notice] = 'Acquisition was successfully updated.'
        format.html { redirect_to(@acquisition) }
        format.xml  { head :ok }
      else

...
class RegexpMatch
  ...
  module ClassMethods
    ...
    def promote(value, dataToParse)
      RegexpMatch.new(value, dataToParse)
    end # promote
  end # ClassMethods
  extend ClassMethods
  ...
  def initialize(regexp_tree, dataToParse)
    ...
    if @regexp_tree.to_regexp.nil?
      @errors = ['@regexp_tree.to_regexp.nil?']
      @match_data = nil
    else
      @match_data = @regexp_tree.to_regexp.match(@dataToParse)
      if @match_data.nil?
        exact_match_regexp = Regexp.new(Regexp.escape(@dataToParse), Regexp::Default_options)
  ...
  def force
    force_match = clone
    if modification.match_data.nil?
  ...
  def map_matches(branch = self, data_to_match = @dataToParse)
    ...
    if matched_data.nil? || matched_data.empty?
  ...
  def match_branch(branch = self, data_to_match = @dataToParse)
    regexp = if branch.instance_of?(String)
               branch.to_exact_regexp
             else
               branch.to_regexp
             end # if
    @match_data = regexp.match(data_to_match)
    ret = if @match_data
            RegexpMatch.new(branch, @match_data[0])
          else # no match
  ...
  def consecutiveMatches(increment = +1, start_limit = 0, end_limit = size - 1)
    startPos = endPos = if increment == 1
                          start_limit
                        else
                          end_limit
                        end # if
    ...
    end until startPos < 0 || startPos > end_limit || endPos > size
    ret
  ...
  # end_limit - array index into parsedTree to end (inclusive)
  # returns when incremented from startPos/endPos past endPos/startPos
  def consecutiveMatch(increment = +1, start_limit = 0, end_limit = size - 1)
    startPos = endPos = if increment == 1
                          start_limit
                        else
                          end_limit
    ...
    begin # until
      ...
      if matchData # expand selection
        ...
        if lastMatch.nil? # no matches yet
          startPos = endPos += if increment > 0
                                 increment
                               else
                                 increment
                               end

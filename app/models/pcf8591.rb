...
def command_string(command = :get, *args)
 #	puts "in command_string, args="+args.inspect
   command_start = "sudo i2c#{command} -y #{@bus} #{@address}"
...
def adc_read(control_byte = Default_control_byte, burst_length = 1, analog_output_enable = CB::Analog_output_enable)
   control_byte |= CB::Analog_output_enable # default to analog out enabled
  if @last_control_byte != control_byte
  ...
  iterations = if analog_output_enable
     burst_length
  else
     burst_length+MAX_OSC_WARM_UP
               end # if
  iterations.times do |_index| # extra
     sysout = `#{get_value_string}`
#		puts "get_value_string="+get_value_string
  #		puts sysout
    value = sysout.chomp.hex
    if analog_output_enable
       ret.push(value)
    else
       if value != @read_behind
         ret.push(value)
       end # if
    end # if
...
def assert_loopback(input_control_byte, _output_value = 0)
   256.times do |i|
     dac_set(i)

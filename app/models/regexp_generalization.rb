...
class RegexpGeneralization < RegexpMatch # file context
  ...
  def matchSubTree
    if empty?
      return ['.', '*']
    elsif RegexpMatch.match_data?(to_regexp, @dataToParse)
      return self
    elsif is_a?(Array)
      matchedTreeArray
    else
  ...
  def mergeMatches(matches)
    ...
    elsif matches[0].end >= matches[1].begin # overlap
    ...
    elsif matches.size == 2 # no overlap w/2 matches
  ...
  def matchedTreeArray
    if self.class.PostfixOperators.index(self[0])
      return to_s
    else
  ...
  def map_prefix
    ret = yield(self) # end recursion
    if ret.nil?
      return nil
    else
      if is_a?(Array)
        return map do |sub_tree|
                 self.class.new(sub_tree).map_prefix { |p| yield(p) }
               end # map
      else
        yield(self) # end recursion
      end # if
    end # map
  end # map_prefix
end # RegexpMatch

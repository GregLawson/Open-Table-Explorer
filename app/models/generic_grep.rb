...
module GenericGrep
  module ClassMethods
    ...
    def model_grep_command(model_regexp_string)
      unless Generic_Table.rails_MVC_class?(name)
        raise "#{name}.model_grep only works on Rails MVC."
      end # if
      "grep \"#{model_regexp_string}\" #{model_file_name} &>/dev/null"
    end # model_grep_command

    def model_grep(model_regexp_string)
      `#{model_grep_command(model_regexp_string)}`
    end # model_grep

    def association_grep_pattern(model_regexp_string, association_name)
      "#{model_regexp_string}:#{association_name}" # no end of line $, so that polymorphic associations are found.
    end # association_grep_command
    ASSOCIATION_MACRO_LETTERS = '[has_manyoneblgtd]'.freeze
    ASSOCIATION_MACRO_PATTERN = "^[hb]#{ASSOCIATION_MACRO_LETTERS}*\s\s*".freeze
    def grep_all_associations_command
      grep_command(ASSOCIATION_MACRO_PATTERN, 'app/models/*.rb')
    end # grep_all_associations_command

    def all_associations
      regexp = '([a-z_.]*):(' + ASSOCIATION_MACRO_PATTERN[1..-1] + ')(.*)'
      `#{grep_all_associations_command}`.split("\n").map do |l|

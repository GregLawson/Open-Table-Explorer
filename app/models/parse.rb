...
class MatchRefinement < Array
  module DefinitionalClassMethods # if reference by DefinitionalConstants or not referenced
		def minimum_refinements(kind)
			case kind
			when :no_matches then 1
			when :exact then 1
			when :left then 2
			when :right then 2
			when :inside then 3
			when :scattered then 4
			end # case
  ...
  def inspect
    'MatchRefinement ' + kind.inspect + "\n" +
      map do |refinement|
        if refinement.is_a?(MatchCapture)
          if refinement.regexp.nil?
            'regexp is nil'
          elsif refinement.success?
            if refinement.matched_characters == ''
								'no characters matched by optional? ' + refinement.regexp.to_literal
						else
            ...
            if refinement.string == ''
								'no characters matched by optional? ' + refinement.regexp.to_literal
						else
							refinement.string.to_literal +
								' not matched by ' + refinement.regexp.to_literal
						end # if
          end # if
        elsif refinement.is_a?(SplitCapture)
          if refinement.regexp.nil?
            'regexp is nil'
          elsif refinement.success?
            refinement.matched_characters.to_literal +
  ...
  def join # reconstruct both matched and unmatched source string
    map do |refinement|
      if refinement.is_a?(MatchCapture)
				if refinement.success?
					refinement.matched_characters
				else
					refinement.string # maybe repeated, messing up join == string
				end # if
  ...
  def capture_indices
    (0..(size - 1)).select do |refinement_index|
      self[refinement_index].is_a?(MatchCapture) && self[refinement_index].success?
    end # each
  end # capture_indices
  ...
  def unmatched_indices
    (0..(size - 1)).map do |refinement_index|
      if self[refinement_index].is_a?(MatchCapture)
  ...
  def error_message(explain_kind = kind)
    case explain_kind
    ...
    when :scattered then 'An scattered refinement requires all regexp to succeed but interrupted by a residual.'
      ...
      raise explain_kind.inspect + ' unexpected'
    end # case
  end # error_message
  ...
  module Assertions
    ...
    def assert_pre_conditions(message = '')
      ...
      each do |refinement|
				message += 'refinement = ' + refinement.inspect
        if refinement.is_a?(MatchCapture)
...
#!			assert_operator(MatchRefinement.minimum_refinements(kind), :<=, self.size)
...
#!			assert_operator(MatchRefinement.minimum_refinements(desired_kind), :<=, self.size)
...
module TimeTypes
  ...
  Git_show_medium_timestamp_regexp_array = [Week_day_regexp,
                                            ...
                                            Space_regexp * Time_regexp, Space_regexp * Year_regexp, Space_regexp * Timezone_number_regexp].freeze
  ...
  Git_reflog_timestamp_regexp_array = [Week_day_regexp,
                                       ...
                                       Space_regexp * Hour_regexp * ':' * Minute_regexp * ':' * Second_regexp * Space_regexp * Timezone_number_regexp].freeze
...
class Capture
  ...
  def show_matches(_unmatches, _regexp_array) # match Array of unmatches, allows delimiter recursion.
  ...
  module Assertions
    ...
    def assert_success
      ...
      need_capture = priority_refinements.any? do |refinement|
        refinement.is_a?(Capture)
      end # any?
      assert(need_capture, priority_refinements.inspect)
    end # assert_success
...
class Bisection < Capture
  include Virtus.value_object
  values do
    attribute :left_length, Fixnum, default: ->(bisection, _attribute) { bisection.regexp.size / 2 }
  end # values
...
class RawCapture < Capture
  ...
  def assert_refinement(desired_kind, message = '')
    ...
    message += priority_refinements.ruby_lines_storage + "\n"
		if desired_kind == :scattered
			assert_operator(MatchRefinement.minimum_refinements(desired_kind), :<=, priority_refinements.size, message)
		else
			if @regexp.instance_of?(Regexp)
#!				assert_equal(MatchRefinement.minimum_refinements(desired_kind), priority_refinements.size, message)
			else
#!				assert_equal(@regexp.size -1 + MatchRefinement.minimum_refinements(desired_kind), priority_refinements.size, message)
			end # if
		end # if
		if @regexp.instance_of?(Regexp)
			case desired_kind
      ...
      when :inside then assert_equal(3, priority_refinements.size, message)
			end # case
		elsif @regexp.instance_of?(Array)
			case desired_kind
      when :no_matches then assert_equal(1, priority_refinements.size, message)
      when :exact then assert_equal(@regexp.size, priority_refinements.size, message)
      when :left then assert_equal(@regexp.size + 1, priority_refinements.size, message)
      when :right then assert_equal(@regexp.size + 1, priority_refinements.size, message)
      when :inside then assert_equal(@regexp.size + 2, priority_refinements.size, message)
      when :scattered then assert_operator(@regexp.size + 3, :<=, priority_refinements.size, message)
			end # case
#!			assert_equal(@regexp.size, priority_refinements.size, message)
		else
			raise 'Unexpected regexp type: ' + message
		end # if
      case desired_kind
      when :no_matches then assert_equal(@string, priority_refinements.join, inspect)
      when :exact then assert_equal(@string, priority_refinements.join, inspect)
      when :left then assert_equal(@string[0, priority_refinements.join.size], priority_refinements.join, inspect)
      when :right then assert_equal(@string[-priority_refinements.join.size, priority_refinements.join.size], priority_refinements.join, inspect)
      when :inside then assert_match(priority_refinements.join, @string, inspect)
      when :scattered then #! refute_equal(priority_refinements.join, @string, inspect)
      else
      		raise 'unknown desired_kind = ' + desired_kind.inspect
      end # case

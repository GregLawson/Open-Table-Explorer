...
Open_tax_solver_data_directory = "#{Open_tax_solver_directory}/examples_and_templates/US_1040".freeze
Open_tax_solver_input = "#{Open_tax_solver_data_directory}/US_1040_Lawson.txt".freeze
Open_tax_solver_sysout = "#{Open_tax_solver_data_directory}/US_1040_Lawson_sysout.txt".freeze

Open_tax_solver_binary = "#{Open_tax_solver_directory}/bin/taxsolve_US_1040_2012".freeze
Command = "#{Open_tax_solver_binary} #{Open_tax_solver_input} >#{Open_tax_solver_sysout}".freeze
OTS_template_filename = "#{Open_tax_solver_data_directory}/US_1040_template.txt".freeze
Symbol_pattern = '^ ?(?<name>[-A-Za-z0-9?]+)'.freeze
Delimiter = '\s+'.freeze
Specific_types = ['\?\?', '0', ';', '0\s+;', 'Yes'].freeze
Type_pattern = '(?<blank_or_outter>\s+|' + Specific_types.map { |a| '(?<outter>' + Delimiter + '(?<type_chars>' + a + ')' + Delimiter + ')' }.join('|') + ')'
Description_pattern = '\{(?<description>.+)\}'.freeze
...
Full_regexp_array = [Symbol_pattern, Type_pattern, Description_pattern].freeze
end # Constants
include Constants
...
def self.coarse_filter
  raw_acquisitions.map do |acquisition|
    matches = acquisition.lines.map do |line|
      if Type_regexp.match(line) && Description_regexp.match(line)
        line
      end # if
    end # each_line
...
def self.all(_tax_year = Default_tax_year)
...
module Examples
  Simple_acquisition = 'L 0 {e}'.freeze
Short_acquisition = 'L  {e}'.freeze
end # Examples
...
def assert_pre_conditions
    ...
    assert_respond_to(attributes, :values)
  ...
  #		assert_includes(NoDB.methods, :insert_sql)
    assert_includes(methods, :insert_sql)
...
def assert_pre_conditions
  ...
  Dir[input_file_names].each do |f|
     assert(File.exist?(f), Dir["#{Open_tax_solver_data_directory}/*"].inspect)
  end # each
end # assert_pre_conditions
...
def assert_full_match(acquisition)
  ...
  if matchData
    ...
    Array.new(capture_kind.size) do |i|
      ...
      if capture_kind[i].instance_of?(String)
         ...
         when -1 then nesting.pop
           ...
           raise nesting.inspect
         end # case
        unless matchData[match_index].nil?
    ...
    when [false, true, true, true, true] then
      ...
      assert_equal(matchData[2], matchData[3] || matchData[5] || matchData[7], matchData.inspect)
       ...
       raise matchMap.inspect + matchData.inspect
    end # case
  else

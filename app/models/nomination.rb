...
class Nomination < Dry::Types::Value
  ...
  extend DefinitionalClassMethods
    ...
    attribute :commit, Types::Strict::Symbol
		...
		attribute :test, Types::Strict::Symbol
#		attribute :ruby_conversion, Types::Strict::String.optional
    ...
    include DefinitionalConstants
		def stash(test_executable)
			if test_executable.repository.something_to_commit?
				...
				Nomination.new(commit: :stash, unit: test_executable.unit, test: test_executable.test)
			else
				nil
			end # if
		end # stash
  ...
  module ReferenceObjects # example constant objects of the type (e.g. default_objects)
    include DefinitionalConstants
		Self = Nomination.new(commit: :stash, unit: TestExecutable::Examples::TestTestExecutable.unit.model_basename, test: TestExecutable::Examples::TestTestExecutable.test_type)

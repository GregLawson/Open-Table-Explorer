...
module GenericTableAssociation
  module ClassMethods
    ...
    def foreign_key_names
      content_column_names = content_columns.collect(&:name)
    ...
    def is_foreign_key_name?(symbol)
      foreign_key_names.include?(symbol.to_s) && is_association?(foreign_key_to_association_name(symbol))
    ...
    def associated_foreign_key_name(association_referenced_by_foreign_key)
      unless is_association?(association_referenced_by_foreign_key.to_s.singularize)
        raise "Association #{association_referenced_by_foreign_key.to_s.singularize} is not an association of #{name}."
      end # if
      ...
      if matchingAssNames.empty?
        raise "Association #{association_referenced_by_foreign_key} does not have a corresponding foreign key in association #{name}."
      end # if
      matchingAssNames.first
    ...
    def is_matching_association?(association_name)
      if is_association?(association_name)
        ...
        if association_class.is_association?(table_symbol)
          return true
        elsif association_class.is_association?(association_method_symbol(table_name.singularize.to_sym))
          return true
        else
          return false
        end # if
    ...
    def association_methods(association_name)
      matching_instance_methods(association_name, false)
    ...
    def match_association_patterns?(association_name, association_pattern)
      patterns = association_methods(association_name).map do |_n|
    ...
    def is_association?(association_name)
      ...
      if ActiveRecord::Base.instance_methods_from_class.include?(association_name.to_s)
      ...
      if association_name.to_s[-4, 4] == '_ids' # automatically generated
        return false
      elsif instance_respond_to?(association_name) && instance_respond_to?((association_name.to_s + '=').to_sym)
        return true
      else
        return false
      end
    ...
    def is_association_to_one?(association_name)
      if is_association?(association_name) && !instance_respond_to?((association_name.to_s.singularize + '_ids').to_sym) && !instance_respond_to?((association_name.to_s.singularize + '_ids=').to_sym)
        return true
      else
        return false
      end
    ...
    def is_polymorphic_association?(association_name)
      is_association_patterns?(association_name, @@Example_polymorphic_patterns)
    end # is_polymorphic_association

    def association_names_to_one
      instance_methods(false).select { |m| is_association_to_one?(m) }
    end # association_names_to_one

    def association_names_to_many
      instance_methods(false).select { |m| is_association_to_many?(m) }
    end # association_names_to_many

    def association_names
      instance_methods(false).select { |m| is_association_to_one?(m) || is_association_to_many?(m) }
    ...
    def name_symbol(model_name)
      if model_name.is_a?(Class)
        return model_name.name.tableize.to_sym
      elsif model_name.is_a?(String)
        return model_name.tableize.to_sym
      elsif model_name.is_a?(Symbol)
    ...
    def association_method_plurality(association_table_name)
      if instance_respond_to?(association_table_name)
        return association_table_name.to_sym
      elsif instance_respond_to?(association_table_name.to_s.singularize)
        return association_table_name.to_s.singularize.to_sym
      elsif instance_respond_to?(association_table_name.to_s.pluralize)
    ...
    def association_method_symbol(association_table_name)
      association_method_plurality(name_symbol(association_table_name))
    ...
    def association_default_class_name?(association_name)
      ...
      if eval("defined? #{default_association_class_name}")
    ...
    def association_class(association_name)
      if !is_association?(association_method_symbol(association_name))
        raise "#{association_method_symbol(association_name)} is not an association of #{name}."
      elsif is_polymorphic_association?(association_name)
        raise "Polymorphic associations #{association_method_symbol(association_name)} of #{name} do not have a single class.. Need instance not class method "
      else
        default_class_defined = association_default_class_name?(association_name)
        if default_class_defined
          ...
          if all_association_classes.size == 1
    ...
    def association_arity(association_name)
      if is_association_to_one?(association_name)
        return :to_one
      elsif is_association_to_many?(association_name)
        return :to_many
      else
  ...
  def foreign_key_value(association_name)
    self[association_name.to_s + '_id']
  ...
  def association_class(association_name)
    if !is_a?(ActiveRecord::Base)
    ...
    elsif !self.class.is_association?(self.class.association_method_symbol(association_name))
      ...
      if association.instance_of?(Array)
        ...
        return association.enumerate(:map, &:class)
      end # if
   end # if
  ...
  def foreign_key_points_to_me?(ar_from_fixture, association_name)
    ...
    if associated_records.instance_of?(Array)
  ...
  def logical_primary_key_recursive_value(_delimiter = ',')
    if self.class.sequential_id?
      return logical_primary_key_value
    else
      self.class.logical_primary_key.map do |e|
        if self.class.is_foreign_key_name?(e)
  ...
  def logical_primary_key_value(delimiter = ',')
    if self.class.sequential_id?
      if respond_to?(:logical_primary_key) # still sequential, not requred, default
        return self[:created_at] # better sequential key
      else
        return id # logical primary key
      end
    else
      if self.class.logical_primary_key.is_a?(Array)
  ...
  def association_has_data(association_name)
    association_state(association_name)[/ and returns type /, 0]
  end # def

  def associated_to_s(association_name, method, *args)
    if self[association_name.to_s + '_id'].nil? # foreign key uninitialized
      return ''
    else
      ass = send(association_name)
      if ass.nil?
  ...
  module Assertions
    ...
    module ClassMethods
      ...
      def assert_foreign_keys_not_nil
        all.each(&:assert_foreign_keys_not_nil) # each
    ...
    def assert_foreign_key_name(class_reference, foreign_key_name)
      unless class_reference.is_a?(Class)
        class_reference = class_reference.class
      end # if
      unless foreign_key_name.is_a?(String)
        foreign_key_name = foreign_key_name.to_s
      end # if
      assert(class_reference.foreign_key_names.include?(foreign_key_name), "foreign_key_name=#{foreign_key_name} is not a foreign key of class_reference=#{class_reference.inspect}")
    ...
    def assert_association(class_reference, association_reference, message = nil)
      ...
      klass = if class_reference.is_a?(Class)
                class_reference
              else

...
class CodeBase
  ...
  @@TABLE_FINDER_REGEXPS = [
    ...
    { name: :tested, example_pathname: 'test/test_helper.rb', Dir_glob: 'test/[a-zA-Z0-9_]*[.]r[a-z]*', plural: nil, test_type: :both },
  ...
  # Returns all specs
  def self.all
    @@TABLE_FINDER_REGEXPS.map { |spec| CodeBase.new(spec) }
  end # all
  ...
  AFFECTS_EVERYTHING = ['db/schema.rb', 'test/test_helper.rb', 'app/models/global.rb', 'app/models/generic_table.rb'].freeze
  AFFECTS_CONTROLLERS = Dir['app/views/shared/*']

  def self.test_pathname(singular_table, test_type)
  ...
  def self.model_pathname(singular_table)
    "app/models/#{singular_table}.rb"
  end # model_pathname

  def self.unit_sources(singular_table)
    ...
    ["test/unit/#{singular_table}_test.rb"] + common_sources
  end # unit_sources

  def self.controller_sources(singular_table)
    plural_table = singular_table.pluralize
    # commn_sources apply to both unit and functional tests.
    ...
    ["test/functional/#{plural_table}_controller_test.rb"] + sources + ["app/controllers/#{plural_table}_controller.rb", "app/helpers/#{plural_table}_helper.rb"]
  end # controller_sources

  def self.unit_target(singular_table)
    "log/unit/#{singular_table}_test.log"
  end # unit_target

  def self.controller_target(singular_table)
    plural_table = singular_table.pluralize
    "log/functional/#{plural_table}_controller_test.log"
  end # controller_target

  def self.model_spec_symbols
    CodeBase.all.select { |s| s[:test_type] != :both }.map { |s| s[:name] }
  end # model_spec_symbols

  def self.spec_symbols
    CodeBase.all.map { |s| s[:name] }
  end # spec_symbols

  # like ActiveRecord method
  def self.find_by_name(spec_name_symbol)
  ...
  def pathnames_with_models?
    pathnames.all? do |f|
      regexp.match(f).size > 1
    end # select
  end # pathnames_with_models

  # for given spec return models
  def models
    if pathnames_with_models?
      models = pathnames.map do |f|
        model = f[regexp, 1]
        if model.nil?
          raise "pathname=#{f} does not match regexp=#{regexp}"
        end # if
        if self[:plural]
  ...
  def self.uptodate?(target, sources)
    ...
    sources.each do |s|
      ...
      if File.mtime(target) < File.mtime(s)
        return false
      end # if
    end # each
    true
  ...
  def self.not_uptodate_sources(target, sources)
  ...
  def self.why_not_stage_helper(pathname, target, sources, test_type)
    if File.exist?(target)
      # ~ puts "Target #{target}  does exist."
      if TestRun.log_passed?(target)
        system "git add #{target}"
        if pathname == target
          return true
        elsif uptodate?(target, [pathname])
          if sources.include?(pathname)
  ...
  # stage target and source pathnames when all tests pass.
  # stage model pathname and .yml pathnames when BOTH unit and controller tests pass
  def self.why_not_stage(pathname, singular_table)
    ...
    if match_spec.nil?
      ...
      if File.exist?(controller_target(singular_table))
...
class MatchedPathName # not < CodeBase see self[:spec]
  ...
  def self.all_tests(test_type)
    test_matches = all.select { |match| match[:test_type] = test_type }
    ret = test_matches.map(&:suggest_test_runs).flatten
    ret
  end # all_tests
  ...
  def suggest_test_runs
    ...
    if self[:matchData].nil?
      all_tests = MatchedPathName.all_tests(test_type)
    else
      case self[:spec][:test_type].to_sym
      when :unit
        TestRun.new(test_executable: TestExecutable.new(:unit, model_name[:singular_model_name], model_name[:plural]))
      when :controller
        TestRun.new(test_executable: TestExecutable.new(:controller, model_name[:singular_model_name], model_name[:plural]))
      when :both
        [TestRun.new(test_executable: TestExecutable.new(:unit, model_name[:singular_model_name], model_name[:plural])),
         TestRun.new(test_executable: TestExecutable.new(:controller, model_name[:singular_model_name], model_name[:plural]))]
      else
  ...
  def self.test_schedule
    MatchedPathName.all.suggest_test_runs.each do |candidate|
      if candidate.instance_of?(Array)
        ret = candidate.first(&:up_to_date?)
      else
  ...
  def self.schedule_tests
    MatchedPathName.test_schedule.each do |tr|
      unless tr.up_to_date? # test updated earlier in this iteration
        tr.run
      end # if
    end # each
  end # schedule_tests
...
class ModelName
  ...
  def initialize(model_name, plurality = nil)
    super()
    if model_name.instance_of?(MatchedPathName)
  ...
  def singular_model_name
    if self[:singular_model_name].nil?
  ...
  def grep_controller_scaffold_variables(_plural_model_name)
    pattern = '(\w+)\.all'
    controller_spec = CodeBase.find_by_name(:controllers)
    controller_pathnames = controller_spec.pathnames
    pattern = '(\w+)\.all'

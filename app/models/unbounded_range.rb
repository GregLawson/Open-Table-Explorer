...
class UnboundedRange < Range
  ...
  def eql?(rhs)
    ...
    if lhs.first.to_i == rhs.first.to_i && lhs.last.to_i == rhs.last.to_i
      true
    else
      false
     end # if
  ...
  def <=>(rhs)
    ...
    if eql?(rhs)
      return 0
    elsif lhs.first <= rhs.first && (rhs.last <= lhs.last)
      return 1
    elsif rhs.first <= lhs.first && (lhs.last <= rhs.last)
      return -1
    else
      return nil
     end # if
  ...
  def +(rhs)
    ...
    UnboundedRange.new(lhs.first + rhs.first, max)
  end # plus

...
class Samba
  module Constants
    Default_server = '192.168.0.12'.freeze
  ...
  module ClassMethods
    ...
    def servers(_workgroup)
      Smb_servers.output.parse(Smb_tree_regexp)
    end # servers

    def tree(_workgroup, _server)
  ...
  def state?
    ...
    when // then
      ...
      raise Exception.new(file_run.inspect)
    end # case
  end # state?
  ...
  def mount_with_mkdir
    case state?
    when :no_mount_point
      puts "!File.exists?(#{@mount_point}) = " + (!File.exist?(@mount_point)).inspect
      puts "File.exists?(#{@mount_point}) = " + File.exist?(@mount_point).inspect
      puts "#{@mount_point} = " + @mount_point
      mkdir_run = ShellCommands.new('mkdir ' + @mount_point)
      unless mkdir_run.success?
        return mkdir_run
      end # if
    end # if

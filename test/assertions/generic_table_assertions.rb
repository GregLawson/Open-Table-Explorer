...
module GenericTableAssertion
  ...
  module KernelMethods
    def association_refs(class_reference = @@example_class_reference, association_reference = @@example_association_reference)
      ...
      yield(class_reference, association_reference)
    end # association_refs

    # assertions testing single generic_table methods
    def all_foreign_key_associations
      Generic_Table.rails_MVC_classes.each do |class_with_foreign_key|
        class_with_foreign_key.foreign_key_association_names.each do |foreign_key_association_name|
          yield(class_with_foreign_key, foreign_key_association_name)
        end # each
      end # each
    ...
    def assert_associations(ass1, ass2, message = nil)
      ...
      if class1.association_default_class_name?(ass2)
    ...
    def assert_general_associations(table_name)
      fixtures(table_name).each_value do |fixture|
        fixture.class.association_names.each do |association_name|
          ...
          if fixture.class.is_association_to_many?(assName)
    ...
    def assert_table_exists(table_name)
      ...
      assert(Generic_Table.table_exists?(table_name), message)
    ...
    def assert_table(table_name)
      ...
      assert(Generic_Table.is_table?(table_name), message)
    end # assert_table
    ...
    def assert_ActiveRecord_table(model_class_name)
      ...
      assert(Generic_Table.is_ActiveRecord_table?(model_class_name), message)
    ...
    def assert_generic_table(model_class_name)
      ...
      assert(Generic_Table.is_generic_table?(model_class_name), message)
    ...
    def assert_matching_association(table_reference, association_name)
      ...
      message = "Table name #{table_symbol} do not have matching associations (has* declarations) with #{'association_name'.titleize} #{association_name}."
      assert(table_class.is_matching_association?(association_name), message)
    ...
    def assert_has_associations(model_class, message = '')
      ...
      assert(model_class.association_names.empty?, message)
...
module ActiveRecord
  class Base
    ...
    def self.assert_numerical(attribute_name)
      message = build_message(message, 'numerical self=?, attribute_name=?', inspect, attribute_name)
      assert(numerical?(attribute_name), message)
    end # numerical

    def self.assert_probably_numerical(attribute_name)
      message = build_message(message, 'probably_numerical self=?, attribute_name=?', inspect, attribute_name)
      assert(probably_numerical?(attribute_name), message)
    end # probably_numerical

    def self.assert_categorical(attribute_name)
      message = build_message(message, 'categorical self=?, attribute_name=?', inspect, attribute_name)
      assert(categorical?(attribute_name), message)
    end # categorical

    def self.assert_probably_categorical(attribute_name, message)
      message = build_message(message, 'probably_categorical self=?, attribute_name=?', inspect, attribute_name)
      assert(probably_categorical?(attribute_name), message)

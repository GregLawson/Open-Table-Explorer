...
class IwTest < TestCase
  ...
  if /DHCP client not found, please set this in the preferences/ =~ wifi_radar_log.output
  ...
  scan_array.each do |bss|
    ...
    if essid_matchData
      if BSS.nil?
        ...
        puts "scan matches ESSID=#{ESSID} but not BSS=#{BSS}, bss=#{bss}" unless ESSID_BSS_match_found
      end # if
    else
      if BSS.nil?
  ...
  if /Note: Host seems down. If it is really up, but blocking our ping probes, try -Pn/ =~ nmap_A.output
    nmap_blocked = ShellCommands.new('nmap -Pn 172.31.42.254')
    if /RTTVAR has grown to over/ =~ nmap_blocked.output
      puts 'RTTVAR has grown to over, suggests bad network.'
    elsif /Host is up / =~ nmap_blocked.output
  ...
  if /No broadcast interfaces found - exiting./ =~ dhclient.output

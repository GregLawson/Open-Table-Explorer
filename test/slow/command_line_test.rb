...
class CommandLineTest < TestCase
  ...
  Test_unit_commandline = CommandLine.new(executable: Test_unit.model_test_pathname?, unit_class: Test_unit.model_class?, argv: ['error_score?', $PROGRAM_NAME])
  Not_virtus_unit = RailsishRubyUnit.new(model_basename: :unit_maturity)
  Not_virtus_unit_commandline = CommandLine.new(executable: Not_virtus_unit.model_test_pathname?, unit_class: Not_virtus_unit.model_class?, argv: ['help', $PROGRAM_NAME])
...
#        test_run = ShellCommands.new('ruby -W0 script/command_line.rb ' + args)
  ...
  def test_executable_object
    ...
    test_run_object = CommandLine.new(executable: TestExecutable.new_from_path($PROGRAM_NAME))
    #	assert_equal(test_run_object.methods, Test_unit_commandline.executable_object.methods)
    #	assert_equal(test_run_object, Test_unit_commandline.executable_object)
    ...
    assert_equal($PROGRAM_NAME, test_run_object.executable.argument_path.relative_pathname.to_s)
    assert_equal($PROGRAM_NAME, Test_unit_commandline.executable_object.test_executable.argument_path.relative_pathname.to_s)

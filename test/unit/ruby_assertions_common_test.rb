...
class RubyAssertionsCommonTest < TestCase
  ...
  def test_assert_subset
    ...
    superset = Set.new(superset_enumeration.to_a.map(&:to_s))
  ...
  def test_equal_sets
    ...
    expected_set = Set.new(expected_enumeration.to_a.map(&:to_s))
    assert(!actual_enumeration.instance_of?(Set))
    actual_set = Set.new(actual_enumeration.to_a.map(&:to_s))
  ...
  def test_global_name
    ...
    assert(global_name?(:RubyAssertionsCommonTest), Module.constants.inspect)
    #	assert(global_name?(:DefaultAssertions), Module.constants.inspect)
  end # global_name

  def test_assert_scope_path
    assert_scope_path(:RubyAssertionsCommonTest, :TestClass)
    assert_scope_path(:TestClass)
  end # assert_scope_path

  def test_assert_path_to_constant
    assert_path_to_constant(:RubyAssertionsCommonTest, :TestClass, :TestConstant) # global path

...
class BugTest < ActiveSupport::TestCase
  ...
  def test_initialize
    ...
    testRun = TestRun.new(test_executable: TestRun.new(test_type, singular_table, plural_table, nil))
    ...
    errors.each do |error|
      ...
      refute_nil(match_data, "error=#{error.inspect}")
      ...
      assert_includes(@error_type, %w(Error Failure))
      puts "@error='#{@error}'"
      bug = Bug.new(test_type, singular_table, error)
      refute_nil(bug)
      refute_nil(bug[:id])
      refute_nil(bug[:gui])
      refute_nil(bug[:resolution])
      refute_nil(bug[:created_at])
      refute_nil(bug[:updated_at])
      refute_nil(bug[:error_type_id])

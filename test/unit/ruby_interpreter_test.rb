...
def test_file_bug_reports
	header,errors,summary=RubyInterpreter.parse_log_file(Odd_plural_testRun.log_file?)
	...
	sysout,run_time=RubyInterpreter.parse_header(header)
	assert_instance_of(Array, sysout)
...
#	puts "blocks='#{blocks.inspect}'"
	...
	assert_not_nil(run_time)
	assert_operator(run_time, :>=, 0)
	header,errors,summary=RubyInterpreter.parse_log_file(testRun.log_file?)
...
def test_initialize
	testRun=RubyInterpreter.new
#	RubyInterpreter.column_names.each do |n|
...
def test_run
	assert_equal("test/unit/test_run_test.rb\n", RubyInterpreter.new(test_command: 'echo', options: '').run.output)
	...
	assert_match(ruby_pattern, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	assert_match(parenthetical_date_pattern, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	assert_match(bracketed_os, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	assert_match(ruby_pattern * parenthetical_date_pattern, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	assert_match(parenthetical_date_pattern * bracketed_os, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	assert_match(version_pattern, RubyInterpreter.new(test_command: 'ruby', options: '--version').run.output)
	output = RubyInterpreter.new(test_command: 'ruby', singular_table: 'unit').run.assert_post_conditions.output
	unit_run = RubyInterpreter.new(test_command: 'ruby', singular_table: 'unit').run

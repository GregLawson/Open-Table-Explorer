...
class DefinitionsTest < TestCase
  ...
  def test_CLASS_constants
    ...
    assert(File.exist?(OTF_SQL_dump_filename), Dir["#{Data_source_directory}/*"].inspect)
...
class TransformsTest < TestCase
  ...
  def test_parse
    model_class?.raw_acquisitions.each do |r|
      assert_equal(%w(form title year pdfSum fields), JSON[r].keys)
    end # each
    hash = { :year => Default_tax_year, :form => 'f1040', 'fields' => { 'L1' => 'Amount' } }
    assert_instance_of(Hash, hash)
    acquisition = JSON[hash]
...
class PjsonsTest < TestCase
  ...
  def test_parse
    ...
    model_class?.raw_acquisitions.each_with_index do |acquisition|
      begin
        ...
        assert(!acquisition.empty?, 'acquisition should not have a size of zero at start of parse')
      ...
      assert(acquisition.nil? | acquisition.empty? | acquisition.empty?)
      array_of_hashes
    end.flatten # map
  ...
  def test_match_regexp_array
    ...
    combination_indices.each_cons(2) do |pair|
      ...
      if matchData = /#{regexp_string}/.match(acquisition)

...
class GlobalTest < TestCase
  ...
  def test_noninherited_public_instance_methods
    ...
    assert_equal(Set.new(%w(publicInstanceMethod protectedInstanceMethod)), Set.new(TestClass.instance_methods(false)))
  ...
  def test_noninherited_modules
    ...
    assert_includes('Generic_Table', AcquisitionStreamSpec.ancestors.map(&:name))
    ...
    assert(AcquisitionStreamSpec.ancestors.map(&:name).include?('Generic_Table'), "Module not included in #{canonicalName} context.")
    assert(AcquisitionInterface.ancestors.map(&:name).include?('Generic_Table'), "Module not included in #{canonicalName} context.")
  ...
  def test_Acquisition_Interface_modules
    ...
    assert_includes('Generic_Table', AcquisitionInterface.ancestors.map(&:name))

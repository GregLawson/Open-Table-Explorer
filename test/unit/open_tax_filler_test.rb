...
def test_raw_acquisitions
	...
	all=all_files.map do |filename|
		...
		otff=OpenTaxFormFiller.parse(json_string)
	end.compact # map
	raw_acquisitions=OpenTaxFormFiller.raw_acquisitions
	assert_instance_of(Array, raw_acquisitions, raw_acquisitions.inspect)
	assert_instance_of(OpenTaxFormFiller, raw_acquisitions[0], raw_acquisitions[0].inspect)
	assert_equal(867, OpenTaxFormFiller.raw_acquisitions.size, raw_acquisitions.inspect)
...
def test_coarse_filter
	...
	assert_operator(867, :==, OpenTaxFormFiller.coarse_filter.size, OpenTaxFormFiller.coarse_filter.inspect)
end #coarse_filter
def test_coarse_rejections
	...
	assert_operator(0, :==, OpenTaxFormFiller.coarse_rejections.size, OpenTaxFormFiller.coarse_rejections.inspect)
end #coarse_rejections
def test_all
	assert_operator(867, :==, OpenTaxFormFiller.all.size, OpenTaxFormFiller.fine_rejections.inspect)

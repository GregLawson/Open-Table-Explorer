...
class CodeBaseTest < TestCase
  ...
  def test_pathname_glob
    ...
    assert_equal('test/*[.]r*', CodeBase.find_by_name(:tested).pathname_glob)
  end # pathname_glob
  ...
  def test_spec_symbols
    ...
    assert_equal_sets([:models, :tested, :unit_tests, :functional_tests, :unit_test_logs, :functional_test_logs, :index_partials, :form_partials, :show_partials, :edit_views, :index_views, :new_views, :show_views, :shared_partials, :controllers], CodeBase.spec_symbols)
  end # spec_symbols
  ...
  def pathnames_with_models?
    ...
    assert_equal(false, CodeBase.find_by_name(:tested).pathnames_with_models?)
  ...
  def test_models
    ...
    CodeBase.all.each do |spec|
      ...
      models = pathnames.map do |f|
        ...
        if model.nil?
          assert_includes(spec[:name], [:shared_partials, :tested], "model should not (regexp=#{spec.regexp}) be embedded in pathname (#{f}).")
        else
  ...
  def test_git_status
    ...
    CodeBase.gitStatus { |_status, pathname| pathname }
    CodeBase.gitStatus { |_status, pathname| MatchedPathName.new(pathname) }
  ...
  def test_example_pathnames_exist
    CodeBase.all.each do |spec|
      ...
      assert(File.exist?(example_pathname))
      unless example_pathname.match(spec.regexp)

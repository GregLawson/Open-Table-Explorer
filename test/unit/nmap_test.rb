...
class NmapTest < TestCase
  ...
  def setup # type preconditions
    refute_nil(Network::My_IP, Network::My_IP.inspect)
    refute_nil(Eth0_ip, My_host_nmap.inspect)
    refute_nil(My_host_nmap.xml['nmaprun'], My_host_nmap.inspect)
  ...
  def test_assert_xml
    assert_equal(%w(scaninfo verbose debugging host runstats scanner args
                    start startstr version xmloutputversion), My_host_nmap.xml['nmaprun'].keys)
    assert_equal(%w(type protocol numservices services), My_host_nmap.xml['nmaprun']['scaninfo'].keys)
    assert_equal(%w(scaninfo verbose debugging host runstats scanner args
                    start startstr version xmloutputversion), My_host_nmap.xml['nmaprun'].keys)
    assert_equal(%w(scaninfo verbose debugging runstats scanner args
    ...
    assert_equal(%w(addr addrtype), My_host_nmap.xml['nmaprun']['host']['address'].keys)
    assert_equal(%w(extraports port), My_host_nmap.xml['nmaprun']['host']['ports'].keys)

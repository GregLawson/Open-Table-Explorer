...
class ColumnGroupTest < TestCase
  ...
  assert_includes(ColumnGroup, included_modules)
  ...
  def test_defaulted_primary_logical_key
    ...
    assert_includes('Generic_Table', self.class.included_modules.map(&:name))
  ...
  def test_logical_primary_key
    CodeBase.rails_MVC_classes.each do |model_class|
      if model_class.defaulted_primary_logical_key?
  ...
  def test_candidate_logical_keys_from_indexes
    ...
    CodeBase.rails_MVC_classes.each do |model_class|
      ...
      if indexes.select(&:unique).size >= 2
  ...
  def test_numerical
    ...
    bug_module_names = Bug.included_modules.map(&:name).grep(/Generic/)
  ...
  def test_column_symbols
    column_symbols = StreamPattern.column_names.map(&:to_sym)
  ...
  def test_history_type
    history_types_used = CodeBase.rails_MVC_classes.map(&:history_type?).uniq # map.uniq
  ...
  def test_sequential_id
    ...
    CodeBase.rails_MVC_classes.each do |model_class|
      ...
      if model_class.sequential_id?
        ...
        assert_operator(id_range, :<, 100_000, message)
      else
        assert_operator(id_range, :>, 100_000, "#{model_class.name}.yml probably defines id rather than letting Fixtures define it as a hash.")
        ...
        if model_class.logical_primary_key.is_a?(Array)
          model_class.logical_primary_key.each do |k|
            assert_includes(k.to_s, model_class.column_names)
          end # each
        else

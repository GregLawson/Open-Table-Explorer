...
class RegexpAlternativeTest < TestCase
  ...
  def test_alternatives
    ...
    assert_equal(%w(0 1 2), cc_comparison)
    assert_equal(%w(0 1 2), cc_comparison)
    # Unroll recursion
    #
    ...
    refute_nil(rhs)
    assert_equal(['c'], rhs)
    assert_equal(%w(b c), ([lhs] + rhs).sort)
    ...
    refute_nil(rhs)
    assert_equal(%w(b c), rhs)
    assert_equal(%w(a b c), ([lhs] + rhs).sort)
    # test known cases of method (post conditions)
    assert_equal(%w(a b), RegexpTree.new('a|b').alternatives?)
    assert_equal(%w(a b c), RegexpTree.new('a|b|c').alternatives?)
    assert_equal(%w(a b c d), Alternatives_4.alternatives?)
  ...
  def test_string_of_matching_chars
    ...
    assert_equal(%w(1 2 3), ('1'..'3').to_a)
    assert_equal(%w(A B C), ('A'..'C').to_a)
    assert_equal(%w(Q R S), ('Q'..'S').to_a)
    assert_equal(%w(a b c), ('a'..'c').to_a)
    matches = (('1'..'3').to_a.select do |char|
    ...
    assert_equal(%w(1 2 3), Tree123.string_of_matching_chars)
    ...
    assert_equal(256, RegexpTree.new(RegexpParse::TestCases::Any_binary_char_string.to_s).string_of_matching_chars.length)
    assert_equal(256, RegexpRepetition::Any.string_of_matching_chars.length)
    assert_instance_of(Array, RegexpRepetition::Any.repeated_pattern.string_of_matching_chars)

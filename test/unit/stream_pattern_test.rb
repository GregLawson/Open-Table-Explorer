...
class StreamPatternTest < TestCase
  ...
  def test_aaa_test_new_assertions_ # aaa to output first
    ...
    if Generic_Table.is_generic_table?(@stream_pattern.class.name)
      ...
      if @stream_pattern.association_has_data(:stream_pattern_arguments)
        if StreamPatternArgument.find_all_by_stream_pattern_id(@stream_pattern.id)

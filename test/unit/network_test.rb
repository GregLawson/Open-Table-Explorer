...
def test_NetworkInterface
	lines=Parse.parse_into_array(IFCONFIG.output, Parse::Constants::LINE)
	...
	words=Parse.parse_into_array(double_lines[0], Parse::Constants::WORD)
...
#	assert_equal('Link', words[1], "words=#{words.inspect}, lines=#{lines.inspect}")
	...
	words=double_lines.map do |row|
		words=Parse.parse_into_array(row, Parse::Constants::WORD)
		puts "words=#{words.inspect}, row=#{row.inspect}"
		assert_match(words[0], /eth0|lo|wlan0/, "row=#{row.inspect}, words=#{words.inspect}")
	end #map
	Parse.parse_into_array(IFCONFIG.output, Parse::Constants::LINES).map  do |row| 
		Parse.parse_into_array(row, Parse::Constants::WORD)
	end #map

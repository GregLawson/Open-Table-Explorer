...
def test_model_basename
	assert_equal('test/unit/test_environment_test.rb', $0)
	assert_equal('test/unit/test_environment_test.rb', __FILE__)
	assert_equal('test/unit', File.dirname($0))
	assert_equal('test_environment_test.rb', File.basename($0))
	assert_equal('.rb', File.extname($0))
	assert_equal('test_environment_test', File.basename($0, '.rb'))
	assert_equal('test_environment', File.basename($0, '.rb')[0..-6])
	assert_equal('test_environment', model_basename?)
end #model_basename
def test_class_name
	assert_equal('TestEnvironment', class_name?)
end #class_name
def test_name_of_test
	...
	assert_equal('TestEnvironmentTest', name_of_test?)
end #name_of_test
def test_initialize

...
class NestedArrayTest < TestCase
  ...
  def test_map_recursive
    ...
    assert_equal(%w(1 2), NestedArray.new(%w(1 2)).map_recursive { |p| p })
    assert_equal(%w(String String), NestedArray.new(%w(1 2)).map_recursive { |p| p.class.name })
    assert_equal(['?1', '?2'], NestedArray.new(%w(1 2)).map_recursive { |p| '?' + p })
    assert_equal(['*', '*'], NestedArray.new(['*', '.']).map_recursive { |_p| '*' })
    assert_equal([%w(String String)], NestedArray.new([%w(1 2)]).map_recursive { |p| p.class.name })
    assert_equal([%w(1 2)], NestedArray.new([%w(1 2)]).map_recursive { |p| p })
    ...
    visit_proc = proc { |parseTree| parseTree }
    #	assert_equal('*', NestedArray::Examples::Echo_proc.call)
    assert_equal(['*', '.'], NestedArray::Examples::Echo_proc.call(['*', '.']))
  ...
  def test_map_branches
    ...
    assert_equal(['C', ['.', '*']], NestedArray.new([['*', '.'], 'C']).map_branches(&:reverse))
    assert_equal('*', ['*', '1', '2'][0])
    ...
    assert_equal(['C', ['.', '*'], 'K'], NestedArray.new(['K', ['*', '.'], 'C']).map_branches(&:reverse))
    ...
    assert_equal([['.', '*']], NestedArray.new([['*', '.']]).map_branches(&:reverse))

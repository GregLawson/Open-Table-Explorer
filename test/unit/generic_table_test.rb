...
#  include DefaultTests2
      ...
      if n == 1
        ...
        if x < min
          ...
          min_key = if has_id
                      ...
                      row.logical_primary_key_value_recursive
                        end # if
  ...
  def test_Generic_Table
    ...
    assert(Acquisition.new.is_a?(ActiveRecord::Base))
    model_class = eval('Acquisition')
    assert(model_class.new.is_a?(ActiveRecord::Base))
    ...
    assert(Generic_Table.rails_MVC_class?(:stream_pattern))
    assert_includes('StreamMethod', CodeBase.rails_MVC_classes.map(&:name))
    assert(CodeBase.rails_MVC_classes.map(&:name).include?('StreamMethod'))
  ...
  def test_default_names
    ...
    assert_equal(%w(Col_0 Col_1), Example.class.default_names([1, 2]))
  end # default_names
  ...
  def test_assign_attribute
    ...
    assert_equal(%w(input output), Example.keys.sort)
  end # []

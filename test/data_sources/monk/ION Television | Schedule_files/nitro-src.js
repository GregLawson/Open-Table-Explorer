...
if (typeof NitroJSConnector=="undefined") {
	document.write('<script type="text/javascript" src="' + nitroProtocol + '://assets.bunchball.net/scripts/JSConnector/' + nitroLibsVersion + '/NitroJSConnector.js"><\/script>');
}
if (typeof NitroNotifier=="undefined") {
	document.write('<script type="text/javascript" src="' + nitroProtocol + '://assets.bunchball.net/scripts/notifier/' + nitroLibsVersion + '/NitroNotifier.js"><\/script>');
}
if (typeof Nitro_Widget=="undefined") {
	Nitro_Widget = "loading";
	document.write('<script type="text/javascript" src="' + nitroProtocol + '://assets.bunchball.net/widgets/embed/' + nitroLibsVersion + '/nitroWidget_new.js"><\/script>');
}
...
if (typeof Nitro=="undefined") {
	...
	function Nitro(connectionParams) {
		...
		this.showPendingNotifications = function(callback, asyncToken, returnCount, locale) {
			NitroNotifier.jsConnector = this.jsConnector;
			return NitroNotifier.showPendingNotifications(this.connectionParams, callback, asyncToken, returnCount, locale);
		}
			...
			for (var i = 0; i < children.length; i++) {  
				...
				if (this.isNitroNode(child, "notification")) {			
					...
					if (child.getAttribute("type") != null && child.getAttribute("type") == "date") {
		                ...
		                var catalog = child.getAttribute("catalog");
		                if (size != null) {
		                	src = src + "&size=" + size
		                }
		                if (catalog != null) {
		...
		this.showPaymentDialog = function(params) {
			...
			var apiCallStr = "method=site.getPaymentOptions&verifyPointCategory=true&pointCategory=" + this.paymentDialogParams.pointCategory;
			if (typeof this.paymentDialogParams.siteApiKey != "undefined") {
				apiCallStr += "&siteApiKey=" + this.paymentDialogParams.siteApiKey;
			}
			this.jsConnector.callAPI(apiCallStr, "Nitro.processPaymentOptions", this.counterId, true);
		}
		...
		this.updatePaymentDialogPointsBalance = function(data) {
			...
			var iconImg = document.getElementById('nitro_payment_dialog_points_balance_pc');
		...
		this.showPaymentDialogWithOptions = function(data) {
			...
			var paypal_content = "<div style=\\\'margin:0px 5px 5px 10px\\\'>" + 
				...
				amountHTML + itemHTML + 
				'<input type="hidden" name="custom" value="'+(typeof nitroToolbar != "undefined" && nitroToolbar.args.siteId ? 'toolbarSiteId='+nitroToolbar.args.siteId+'&affl='+nitroToolbar.args.siteId+'&' : '')+'apiKey=' + this.connectionParams.apiKey + '&userId=' + this.jsConnector.connectionParams.userId + '&pointCategory=' + this.paymentDialogParams.pointCategory + (this.paymentDialogParams.siteApiKey != "undefined" ? '&siteApiKey='+this.paymentDialogParams.siteApiKey : '')+ '">' +
			...
			/* Taking out Boku payment option
		...
		this.handleRedirects = function() {
			...
			if(action) {
				...
				if(actionAlreadyLogged) {
					...
					this.jsConnector.callAPI("method=user.clientLogAction&userId=" + userId + "&tags=" + action, "Nitro.handleClientLogAction", this.counterId, true, true);
				}
			}
		}
